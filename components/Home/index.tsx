...
export function HomeComponent() {
  ...
  const getTabClasses = (selected: boolean) =>
    ...
    } px-4 py-2 text-base border-r-2`;
  ...
  const onRenderTabs = () => {
    ...
    return tabList.map((tab) => (
      <Tab
        key={tab.name}
        className={({ selected }) =>
          `${getTabClasses(selected)} flex items-center gap-5 py-3 hover:bg-white/10`
        }
      >
        {tab.icon}
        <p>{tab.name}</p>
  ...
  return (
    <div>
      ...
      >
        <TabList className="flex min-w-[15%] flex-col bg-tapps-light-black">
          {onRenderTabs()}
        </TabList>
        <TabPanels>
